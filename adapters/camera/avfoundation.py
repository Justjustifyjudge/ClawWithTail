"""
macOS camera adapter using AVFoundation backend via OpenCV.
"""
...
class AVFoundationCameraAdapter(CameraAdapter):
    """Camera adapter for macOS using the AVFoundation backend."""
    ...
    def capture_frame(self, save_path: str) -> str:
        ...
        try:
            if not cap.isOpened():
                raise RuntimeError(
                    f"AVFoundation: Cannot open camera index {source} "
                    f"(device_id={self.device_config.id})"
                )
            ret, frame = cap.read()
            if not ret or frame is None:
                raise RuntimeError(
                    f"AVFoundation: Failed to read frame from index {source}"
                )
            success = cv2.imwrite(save_path, frame)
            if not success:
                raise RuntimeError(f"AVFoundation: Failed to write JPEG to '{save_path}'")
            logger.debug("AVFoundation: Frame saved to %s", save_path)
            return save_path
        finally:
            cap.release()
