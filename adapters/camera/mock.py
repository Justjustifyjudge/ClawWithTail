"""
Mock camera adapter — used when no real camera is available.
Returns a minimal valid JPEG (1x1 black pixel) for testing.
"""
from __future__ import annotations

import logging
from pathlib import Path

from adapters.camera.base import CameraAdapter
from core.config.models import DeviceConfig
from core.models.env_profile import EnvProfile

logger = logging.getLogger(__name__)

# Minimal valid 1x1 black JPEG (bytes)
_MOCK_JPEG = bytes([
    ,
])


class MockCameraAdapter(CameraAdapter):
    """
    Mock camera adapter for testing and environments without a real camera.
    Writes a minimal valid JPEG to the requested path.
    """

    def __init__(self, device_config: DeviceConfig, env_profile: EnvProfile) -> None:
        super().__init__(device_config, env_profile)

    def capture_frame(self, save_path: str) -> str:
        Path(save_path).parent.mkdir(parents=True, exist_ok=True)
        Path(save_path).write_bytes(_MOCK_JPEG)
        logger.debug("MockCamera: Wrote mock JPEG to %s", save_path)
        return save_path

    def is_available(self) -> bool:
        return True
